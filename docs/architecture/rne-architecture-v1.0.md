...
**Events produced:** AgentRegistered, AgentActivated, AgentDeactivated, AgentReactivated, AgentQualificationChanged.
...
**Events produced:** CustomerRegistered, CustomerOwnershipAssigned, CustomerOwnershipTransferred, CustomerOwnershipVerified.
...
**Responsibility:** Commission computation, compression, vesting, and payout scheduling.

**Key concepts:** CommissionRun, CommissionLine, CommissionType, CompressionResult.

**Events produced:** CommissionComputed, CommissionAdjusted, CommissionVoided, CommissionRunCompleted.
...
**Responsibility:** Rank definitions, promotion/demotion logic, qualification windows, historical rank tracking.

**Key concepts:** RankDefinition, RankAssignment, QualificationPeriod, RankVolume.
...
Agent {
  ...
  status: AgentStatus [PENDING, ACTIVE, INACTIVE, SUSPENDED, TERMINATED]
...
- When ActiveUntil < current time, status transitions to INACTIVE.
...
- A commission run processes all qualifying transactions within the period.
...
CommissionLine {
  ...
  runId: RunId
  agentId: AgentId
  commissionType: CommissionType [DIRECT, OVERRIDE, BONUS, REWARD]
...
### 4.14 Core Invariants Summary

| Invariant | Enforcement | Violation Consequence |
|---|---|---|
| Customer ownership is permanent | Ownership transfer requires admin event | System integrity failure |
...
| ACTIVE | INACTIVE | ActiveUntil < now | System (scheduled job) |
...
- ACTIVE→INACTIVE is detected by periodic job scanning `activeUntil < now()`.
...
- Ownership may only change via explicit `CustomerOwnershipTransferred` event with admin authorization.
...
- Direct commission is earned regardless of agent status, including INACTIVE and SUSPENDED (but SUSPENDED is held until suspension ends).
...
When a direct agent is INACTIVE (not TERMINATED or SUSPENDED), their override commissions are compressed upward to the nearest qualified active upline.
...
**Rules:**
- Compression only applies to INACTIVE agents. TERMINATED and SUSPENDED agents are not compressed through (see below).
...
An agent must be ACTIVE to earn override commissions.

**Activity definition:**
```
Agent is ACTIVE.defined as:
  agent.status == ACTIVE AND agent.activeUntil >= commissionPeriodEnd
```

**Qualification check:**
```
IsOverrideEligible(agent, periodEnd):
  return agent.status == ACTIVE AND recordExists(
...
Demotion occurs when an agent fails to meet the minimum requirements for their current rank.
...
| AgentReactivated | Agent Context | agentId, reactivationDate, qualifiedSaleId | Network, Commission, Rank, Reward |
| AgentSuspended | Agent Context | agentId, reason, adminId | Commission |
| AgentTerminated | Agent Context | agentId, reason, adminId | Network, Commission |
| AgentQualificationChanged | Agent Context | agentId, metric, newValue, period | Commission, Rank |
...
| CustomerOwnershipTransferred | Customer Context | customerId, previousAgentId, newAgentId, adminId, reason | Commission, Audit |
...
| Event | Producer | Payload | Consumers |
|---|---|---|---|
| SponsorLinkCreated | Network Context | agentId, sponsorId, depth | Commission, Rank |
| SponsorLinkTerminated | Network Context | agentId, reason | Commission |
...
| CommissionComputed | Commission Context | lineId, agentId, amount, rate, sourceTransactionId | Audit, Agent UI |
| CommissionAdjusted | Commission Context | lineId, agentId, adjustmentAmount, reason, adminId | Audit, Agent UI |
| CommissionVoided | Commission Context | lineId, agentId, reversalTransactionId | Audit, Financial Core |

#### Rank Events

| Event | Producer | Payload | Consumers |
|---|---|---|---|
| RankPromoted | Rank Context | agentId, previousRankId, newRankId, promotionDate | Commission, Agent, Audit |
| RankDemoted | Rank Context | agentId, previousRankId, newRankId, demotionDate | Commission, Agent, Audit |
| RankQualificationMet | Rank Context | agentId, rankId, metric, value | Reward, Campaign |
...
| Event | Producer | Payload | Consumers |
|---|---|---|---|
| RewardAwarded | Reward Context | rewardId, agentId, amount, qualificationPeriod | Commission, Audit |
| RewardClaimed | Reward Context | rewardId, agentId, claimedAmount | Payout Engine |
| RewardExpired | Reward Context | rewardId, agentId, reason | Audit |
| RewardProgramActivated | Reward Context | programId, effectiveDate, rules | Commission |

#### Campaign Events

| Event | Producer | Payload | Consumers |
|---|---|---|---|
| CampaignCreated | Campaign Context | campaignId, name, startDate, endDate, rules | Commission, Rank |
...
| CampaignPrizeAwarded | Campaign Context | campaignId, agentId, prize | Audit |

#### Audit Events

| Event | Producer | Payload | Consumers |
|---|---|---|---|
| AuditRecorded | Audit Context | auditId, action, actorId, resourceType, resourceId, previousState, newState | Audit Store |
...
|---|---|---|
| CommissionComputed | 10 years | Hot |
| RankPromoted | Permanent | Hot |
| CustomerOwnershipAssigned | Permanent | Hot |
| AuditRecorded | Permanent | Hot |
| CommissionRunCompleted | 10 years | Warm |
| CampaignPrizeAwarded | 7 years | Warm |
...
| Transfer customer | ADMIN (2-person) | Two administrators must approve |
...
2. Admin B approves transfer (independent review).
3. System creates `CustomerOwnershipTransferred` event.
4. Previous agent is notified.
5. New agent is notified.
6. Commissions from the transfer date forward go to the new agent.
7. Past commissions are not retroactively adjusted (grandfathered).
...
describe("Commission compression"):
  ...
  it("inactive at root: no compression possible, overrides lost")
...
**Decision:** All monetary values are integers (cents). No floating-point arithmetic.

**Rationale:** Floating-point arithmetic produces non-deterministic rounding errors that compound across commission chains. Integer arithmetic is deterministic, auditable, and reproducible. A 0.001 cent rounding difference at a deep level of a large tree can snowball into significant discrepancies.
...
- Residual tracking if needed (largest-remainder method, as in Distribution Engine).
...
**Decision:** Agent status (ACTIVE, INACTIVE) is derived from qualification events, not stored as a mutable field.
...
*This document is the architectural constitution for all RELCKO Network Engine implementation. It supersedes all informal design discussions. Changes to this document require Architecture Board approval.*
